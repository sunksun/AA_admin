<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $pdf->Cell(0,5,iconv( 'UTF-8','TIS-620','ระบบเบิกจ่ายวัสดุอปกรณ์สารเคมีเเละครุภัณฑ์ มหาวิทยาลัยราชภัฏเลย (อาจารย์)'),0,1);
        $pdf->Cell(0,5,iconv( 'UTF-8','TIS-620','ออกรายงาน : '.date('d').'/'.date('m').'/'.(date('Y')+543).' '),0,1);
        $pdf->Ln(5);
        $pdf->SetFont('angsa','',14);
        $pdf->Cell(175,4,iconv( 'UTF-8','TIS-620','ระบบเบิกจ่ายวัสดุอปกรณ์สารเคมีเเละครุภัณฑ์ มหาวิทยาลัยราชภัฏเลย (อาจารย์)'),0,1,'C');
        $pdf->Ln(2);
        ...
        $sql_pdf = "SELECT borrow2.borrow_id , borrow2.approve , borrow2.start_date , borrow2.end_date , borrow_list2.borrow_id , borrow_list2.equ_list , borrow_list2.num , borrow_list2.unit FROM (borrow2 INNER JOIN borrow_list2 ON borrow_list2.borrow_id = borrow2.borrow_id) WHERE borrow2.approve = '2' AND ((start_date BETWEEN '$start_date' AND '$end_date') OR (end_date BETWEEN '$start_date' AND '$end_date'));";
