<?php
	...
	$sql = "UPDATE borrow2 SET approve = ? WHERE borrow_id = ? ;";
	$stmt = $conn->prepare($sql);

	$query2 = mysqli_query($conn, 'SELECT * FROM borrow_list2 WHERE borrow_id = "'.$borrow_id.'"');
	...
	if ($stmt) {
		...
		header("location: officer_approve_return2.php");
	} else {
