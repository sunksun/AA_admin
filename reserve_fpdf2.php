<?php
    ...
    $end_time = $_GET['end_time'];
        ...
        $pdf->Cell(59,6,iconv( 'UTF-8','TIS-620',''.$fullname),0,0,);
        $pdf->Line(20,39,80,39);
        $pdf->Cell(20,7,iconv( 'UTF-8','TIS-620','email'),0,0,);
        $pdf->Cell(40,6,iconv( 'UTF-8','TIS-620',''.$email),0,0,);
        $pdf->Line(95,39,140,39);
        $pdf->Cell(15,7,iconv( 'UTF-8','TIS-620','ที่ทำงาน'),0,0,);
        $pdf->Cell(70,6,iconv( 'UTF-8','TIS-620',''.$dep),0,1,);
        $pdf->Line(150,39,199,39);
        $pdf->Cell(10,9,iconv( 'UTF-8','TIS-620','ตำเเหน่ง'),0,0,);
        ...
        $pdf->Ln();
        // sql 
        $sql_pdf = "SELECT borrow2.approve,borrow2.start_date,borrow2.end_date,borrow2.start_time,borrow2.end_time,borrow_list2.num,borrow_list2.unit, borrow2.id, borrow2.email, borrow2.course , borrow2.borrow_id , borrow_list2.equ_list, tbluser.fullname, tbluser.field, tbluser.dep FROM (borrow2 INNER JOIN borrow_list2  ON borrow2.borrow_id = borrow_list2.borrow_id INNER JOIN tbluser ON borrow2.email = tbluser.email ) WHERE borrow2.id = '$id' AND (borrow2.approve = '2')";
        ...
        $pdf->Cell(30,7,iconv( 'UTF-8','TIS-620','เจ้าหน้าที่'),0,0,'C');
        $pdf->Ln(35);
